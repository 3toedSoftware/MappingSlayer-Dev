(function ( root, factory ) {
...
}(this, function(liblouis, ns) {
    'use strict';

    const API_METHODS = ['version', 'setLogLevel', 'getTable', 'checkTable', 'free',
    ...
    const IS_BROWSER_GUI_THREAD = typeof window !== 'undefined';
    const IS_WEB_WORKER = typeof WorkerGlobalScope !== 'undefined' && self instanceof WorkerGlobalScope;
    const IS_NODE = !IS_BROWSER_GUI_THREAD && !IS_WEB_WORKER;
    ...
    function bindDynLoader(self) {
        ...
        return function dynLoader(parent, name) {
            ...
            if(/(\.cti|\.ctb|\.utb|\.dis|\.uti|\.tbl|\.dic)$/.test(name)) {
                const url = self.capi.TABLE_URL + name;
    ...
    var MEM = {
        ...
        '2': {
            ...
            transfer: function(capi, buff) {
                const bufflen = MEM['2'].getBufferLength(buff);
                const buff_ptr = capi._malloc(bufflen);
            ...
            read: function(capi, buffptr, bufflenptr) {
                if(!bufflenptr) { // null-terminated string
                    ...
                    const start_index = buffptr >> 1;
                    const end_index = start_index + capi.getValue(bufflenptr, 'i32');
                    const outstr_buff = capi.HEAPU16.slice(start_index, end_index);
                    return outstr_buff;
                }
            },
        ...
        '4': {
            ...
            transfer: function(capi, buff) {
                const bufflen = MEM['4'].getBufferLength(buff);
                const buff_ptr = capi._malloc(bufflen);
            ...
            read: function(capi, buffptr, bufflenptr) {
                if(!bufflenptr) { // null-terminated string
                    ...
                    const start_index = buffptr >> 2;
                    const end_index = start_index + capi.getValue(bufflenptr, 'i32');
                    const outstr_buff = capi.HEAPU32.slice(start_index, end_index);
                    return outstr_buff;
                }
            },
    ...
    IMPL.lou = {
        ...
        compileString: function(table, opcodes) {
            const success = this.capi.ccall('lou_compileString', 'number', ['string', 'string'], [table, opcodes]);
            return !!success;
        },
        ...
        translateString: function(table, inbuf, backtranslate) {
            ...
            const inbuff_ptr = this.mem.transfer(this.capi, inbuf);
            const bufflen = this.mem.getBufferLength(inbuf);
            const outbuff_ptr = this.capi._malloc(bufflen);

            const bufflen_ptr = this.capi._malloc(4);
            const strlen_ptr = this.capi._malloc(4);

            this.capi.setValue(bufflen_ptr, bufflen, 'i32');
            this.capi.setValue(strlen_ptr, bufflen, 'i32');

            const success = this.capi.ccall(backtranslate ?
            ...
            const outbuff = this.mem.read(this.capi, outbuff_ptr, bufflen_ptr);
        ...
        enableOnDemandTableLoading: function(tableurl) {
            ...
            if(!IS_NODE) {
                ...
                let tablefolder = tableurl;

                if(!tablefolder) {
                    const path = require('path');
                    const buildpath = require.resolve('liblouis-build');
    ...
    function easyApiDefaultLogCallback(lvl_id, msg) {
        const lvl_name = LiblouisEasyApi.prototype.LOG[lvl_id];
        msg = '[' + lvl_name + '] ' + msg;

        if(typeof console != 'undefined') {
            const fn = console[LiblouisEasyApi.prototype._CONSOLE_MAPPING[lvl_name]];
            if(fn) {
                fn(msg);
            } else {
                console.log(msg);
